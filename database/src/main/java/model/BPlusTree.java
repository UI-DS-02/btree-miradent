...
public class BPlusTree<T> {
     ...
     */
    private int binarySearch(DictionaryPair<T>[] dps, int numPairs, int t) {
        Comparator<DictionaryPair<T>> c = new Comparator<DictionaryPair<T>>() {
            @Override
            public int compare(DictionaryPair<T> o1, DictionaryPair<T> o2) {
                Integer a = o1.key;
                Integer b = o2.key;
                return a.compareTo(b);
            }
        };
        return Arrays.binarySearch(dps, 0, numPairs, new DictionaryPair<T>(t, valueX), c);
    }
     ...
     */
    private LeafNode<T> findLeafNode(int key) {
        ...
        Node<T> child = this.root.childPointers[i];
        if (child instanceof LeafNode) {
            return (LeafNode<T>) child;
        } else {
            return findLeafNode((InternalNode<T>) child, key);
        }
    }

    private LeafNode<T> findLeafNode(InternalNode<T> node, int key) {
		...
		/* Return node if it is a LeafNode object,
		   otherwise repeat the search function a level down */
        Node<T> childNode = node.childPointers[i];
        if (childNode instanceof LeafNode) {
            return (LeafNode<T>) childNode;
        } else {
            return findLeafNode((InternalNode<T>) node.childPointers[i], key);
        }
    }
     ...
     */
    private int findIndexOfPointer(Node<T>[] pointers, LeafNode<T> node) {
        int i;
     ...
     */
    private void handleDeficiency(InternalNode<T> in) {

        InternalNode<T> sibling;
        InternalNode<T> parent = in.parent;
            ...
            for (int i = 0; i < in.childPointers.length; i++) {
                if (in.childPointers[i] != null) {
                    if (in.childPointers[i] instanceof InternalNode) {
                        this.root = (InternalNode<T>) in.childPointers[i];
        ...
        // Borrow:
            ...
            Node<T> pointer = sibling.childPointers[0];
     ...
     */
    private int linearNullSearch(DictionaryPair<T>[] dps) {
     ...
     * @return index of the target value if found, else -1
     */
    private int linearNullSearch(Node<T>[] pointers) {
     ...
     */
    private void shiftDown(Node<T>[] pointers, int amount) {
        @SuppressWarnings("unchecked") Node<T>[] newPointers = new Node[this.m + 1];
     ...
     */
    private void sortDictionary(DictionaryPair<T>[] dictionary) {
        Arrays.sort(dictionary, new Comparator<DictionaryPair<T>>() {
            @Override
            public int compare(DictionaryPair<T> o1, DictionaryPair<T> o2) {
                if (o1 == null && o2 == null) {
                    return 0;
     ...
     */
    private Node<T>[] splitChildPointers(InternalNode<T> in, int split) {

        Node<T>[] pointers = in.childPointers;
        @SuppressWarnings("unchecked") Node<T>[] halfPointers = new Node[this.m + 1];
     ...
     */
    private DictionaryPair<T>[] splitDictionary(LeafNode<T> ln, int split) {

        DictionaryPair<T>[] dictionary = ln.dictionary;

		/* Initialize two dictionaries that each hold half of the original
		   dictionary values */
        @SuppressWarnings("unchecked") DictionaryPair<T>[] halfDict = new DictionaryPair[this.m];
     ...
     */
    private void splitInternalNode(InternalNode<T> in) {
        ...
        Node<T>[] halfPointers = splitChildPointers(in, midpoint);

        // Change degree of original InternalNode in
        in.degree = linearNullSearch(in.childPointers);

        // Create new sibling internal node and add half of keys and pointers
        InternalNode<T> sibling = new InternalNode<T>(this.m, halfKeys, halfPointers);
        for (Node<T> pointer : halfPointers) {
            if (pointer != null) {
        ...
        if (parent == null) {
            ...
            keys[0] = newParentKey;
            InternalNode<T> newRoot = new InternalNode<T>(this.m, keys);
    ...
    public void delete(int key) {
        if (isEmpty()) {
            ...
            LeafNode<T> ln = (this.root == null) ? this.firstLeaf : findLeafNode(key);
            ...
            if (dpIndex < 0) {
                ...
                if (ln.isDeficient()) {
                    ...
                    LeafNode<T> sibling;
                    InternalNode<T> parent = ln.parent;
                    ...
                    if (ln.leftSibling != null &&
                        ...
                        DictionaryPair<T> borrowedDP = sibling.dictionary[sibling.numPairs - 1];
                        ...
                        DictionaryPair<T> borrowedDP = sibling.dictionary[0];
    ...
    public void insert(int key, T value) {
        if (isEmpty()) {
            ...
            LeafNode<T> ln = new LeafNode<T>(this.m, new DictionaryPair<T>(key, value));
            ...
            LeafNode<T> ln = (this.root == null) ? this.firstLeaf :
                    findLeafNode(key);

            // Insert into leaf node fails if node becomes overfull
            if (!ln.insert(new DictionaryPair<T>(key, value))) {

                // Sort all the dictionary pairs with the included pair to be inserted
                ln.dictionary[ln.numPairs] = new DictionaryPair<T>(key, value);
                ln.numPairs++;
                sortDictionary(ln.dictionary);

                // Split the sorted pairs into two halves
                int midpoint = getMidpoint();
                DictionaryPair<T>[] halfDict = splitDictionary(ln, midpoint);
                ...
                if (ln.parent == null) {
                    ...
                    InternalNode<T> parent = new InternalNode<>(this.m, parent_keys);
                    ln.parent = parent;
                ...
                LeafNode<T> newLeafNode = new LeafNode<>(this.m, halfDict, ln.parent);
    ...
    public ArrayList<T> search(int lowerBound, int upperBound) {
        ...
        LeafNode<T> currNode = this.firstLeaf;
        while (currNode != null) {
     ...
     */
    private class InternalNode<E> extends Node<T> {
        int maxDegree;
        int minDegree;
        int degree;
         ...
         *                 childPointers list
         */
        private void appendChildPointer(Node<T> pointer) {
         ...
         */
        private int findIndexOfPointer(Node<T> pointer) {
         ...
         */
        private void insertChildPointer(Node<T> pointer, int index) {
         ...
         */
        private void prependChildPointer(Node<T> pointer) {
         ...
         */
        private void removePointer(Node<T> pointer) {
            for (int i = 0; i < childPointers.length; i++) {
                if (childPointers[i] == pointer) {
         ...
         */
        private InternalNode(int m, Integer[] keys, Node<T>[] pointers) {
            this.maxDegree = m;
            this.minDegree = (int) Math.ceil(m / 2.0);
     ...
     */
    public class LeafNode<E> extends Node<T> {
        int maxNumPairs;
        int minNumPairs;
        int numPairs;
         ...
         */
        public LeafNode(int m, DictionaryPair<T>[] dps, InternalNode<T> parent) {
            this.maxNumPairs = m - 1;
            this.minNumPairs = (int) (Math.ceil(m / 2.0) - 1);
            this.dictionary = dps;
     ...
     */
    public class DictionaryPair<E> implements Comparable<DictionaryPair<T>> {
        int key;
        T value;
